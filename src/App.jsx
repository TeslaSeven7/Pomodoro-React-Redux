...
import { updateInitialState } from './features/chrono';
...
function App(type) {
	...
	function addNewCard(event) {
		if (ref.current.value.trim().length != 0) {
			...
			setErrorMessage('Task needs a label');
			//!POTENTIEL PB
			console.log(event);
	...
	return (
		<div className='bg-gray-200 text-slate-800 pt-20 min-h-screen'>
			<div className='flex flex-col items-center content-center max-w-2xl mx-auto bg-white p-10 border-2 border-black rounded-xl shadow-[2px_2px_0px_0px_#000000] px-[10rem]'>
				...
				<div className='relative mb-5 px-5 py-5 bg-fuchsia-100 shadow-[3px_3px_0px_0px_#e879f9] border border-fuchsia-300 rounded-lg w-full'>
					...
					<p className='absolute bottom-0 text-sm text-orange-600'>
						{errorMessage}
					</p>
				</div>
